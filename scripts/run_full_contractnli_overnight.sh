...
BATCH_SIZE="${3:-25}"
PAUSE_SECONDS="${4:-600}"
RUN_ID="${5:-overnight_lbr_${MODE}_${MODEL}}"
...
print(sum(1 for _ in iter_contractnli_cases()))
PY
)"
...
if command -v caffeinate >/dev/null 2>&1 && [[ "${UNDER_CAFFEINATE:-0}" != "1" ]]; then
  ...
  exec caffeinate -i "$0" "$MODE" "$MODEL" "$BATCH_SIZE" "$PAUSE_SECONDS" "$RUN_ID"
fi
...
run_batches() {
  ...
  for ((offset=0; offset<TOTAL_CASES; offset+=BATCH_SIZE)); do
    ...
    PYTHONPATH=. .venv/bin/python scripts/run_eval_contractnli.py \
      --mode "$MODE" \
      --offset "$offset" \
      --limit "$BATCH_SIZE" \
      --warmup \
